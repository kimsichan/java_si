...
//좌표평면에서 점 하나를 나타내기 위한 클래스 point2D 를 만들어보세요.
/* 클래스명 : point2D 
 * 멤버변수 : 점하나 => x좌표, y좌표 => int x, int y
 * 멤버메소드 : 
 *  - 현재 좌표 정보를 출력하는 메소드
 *  - 지정된 좌표(누군가 알려준 좌표)로 이동
 *  생성자
 *  - 기본생성자 : x,y좌표를 0으로 초rl화
...
class point2D{
	...
	//매개변수 필요 x => 남이 알려준 좌표가 아닌, 내 x,y 좌표를 출력해야 하기 때문
	void print() {
