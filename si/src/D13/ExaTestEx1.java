...
public class ExaTestEx1 {
	...
	public static void main(String[] args) {
		/* 도형을 그리는 기능을 Shape에 추상메소드로 만든경우와 그리는 기능을
		 * Shape에서 제뢰한 클래스의 차이를 보여주는 예제
		 * 부모에 그리는 기능이 있으면, 다향성을 이용하여 조건없이(형변환 확인없이)
		 * 그리는 기능을 사용할 수 있지만, 
		...
		ExbSpape [] shapeList2 = new ExbSpape[3];
		shapeList2[0] = new ExbEllipse();
		shapeList2[1] = new ExbRect();
		shapeList2[2] = new ExbRect();
		
		for(ExbSpape tmp : shapeList2) {
			//ExbSpahe에 print()가 없어서 에러발생
			//tmp.print();
			//((ExbRect)tmp).print();
			if(tmp != null) {
