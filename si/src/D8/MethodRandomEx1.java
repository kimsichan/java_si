...
public class MethodRandomEx1 {
	...
	public static void main(String[] args) {
		...
		//학생이 선생님이 알려준 정수의 범위 안에 있는 정수를 적음
		random3(1,10);
		//학생이 알려준 정수(선생님이 지정한 범위의 정수)를 선생님이 칠판에 적ㅇ므
		System.out.println(random4(1,10));
		
	}
	/* 기능 : 1부터 10사이의 랜덤한 수를 생성하여 출력하는 메소드
	 * 선생님 : 학생1에게  정수를 칠판에 적으세요. 이때 학생이 1에서 10사이의 정수를 칠판에 적음 
	 * 매개변수 : 없음
	 ...
	 * 선생님 : "학생1, 1부터 10사이의 정수를 생각해서 알려주세요"
	 * 학생  : 1에서 10사이의 정수를 말함.
	 * 매개변수 : 1(최소값), 10(최대값) =>int min,int max
	 *        :1과 10이라는 정보를 학생에게 알려줌.
	 * 리턴타입 : 정수=>int
	...
	public static int random4(int min , int max ) {
		int random = (int)(Math.random()*(max - min + 1) + min);
		return random;
		//retuen (int)(Math.random()*(max - min + 1) + min);
	}
}
