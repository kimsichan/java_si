...
public class LottoEx1 {
	...
	public static int createBonus(int min, int max, int []arr) {
		...
		while(true) {
			...
			//당첨번호에 보너스번호가 있으면 조건식으로 점프(이동)하여 다시 번호 생성
			if(containsArray(arr, bonus, arr.length)) {
				continue;
			}
			//당첨번호에 보너스 번호가 없으면 반복문 종효
			break;
		}
		return bonus;
	}
	...
	public static int check(int []arr1, int []arr2) {
		int count =0;
		for(int tmp : arr1) {
			//배열arr2에 배열 arr1에서 꺼낸 tmp가 있으면 count를 1증가
	 ...
	 * 매개변수 : 로또 번호화 보너스 번호, 사용자 번호 => int []lotto, int bonus, int []user 
	 * 리턴타입 : 없음 => void
	 * 메소드명 : rank
	 * */
