...
public class ExeBaseBallEx1 {
	 ...
	 * 		 않게 count개를 저장하는 메소드
	 * 매개변수 : 주어진 정수 리스트, 정수 범의 => List<Integer> list,int min,int max ,int count
	 * 리턴타입 : 없음 => void
	...
	public static void createRandomList( List<Integer> list,int min,int max,int count) {
		...
		//중복되지 않은 숫자를 생성하기 위한 임시set을 선어 및 생성 
...
//					}
	...
	/* 기능 : 정수 리스트가 주어지면 해당 정수 리스텡 min~max 사이의 값을 스캐너를 통해 
	 * 		 count개 입력받아 재대로 입력했는지를 알려주는 메소드
	 *       0 : 재대로 입력,  1 : 범위 잘못, -1 : 중복
	 * 매개변수 : 리스트, 범위, 갯수, 스캐너 =>
