...
- 영화
 - 영화번호(int, pk), 제목, 연령, 런닝타임, 개봉일, 영화상세,
- 영화장르
...
- 참여 
 - 참여번호(pk), 영화번호 (fk), 인물번호(fk), 역활
- 극장 
 - 극장번호(pk), 지역, 지점, 주소, 교통안내, 주차안내, 관 갯수, 총 좌석
- 좌석 
 - 좌석번호(pk), 좌석명, 극장번호(fk), 관번호, 좌석상태, 좌석타입
- 상영시간
 - 상영번호(pk), 영화번호(fk), 극장번호(fk), 상영시간, 관번호, 옵션(2D,3D,4D), 총좌석수, 예약가능좌석수
- 회원
 - 아이디, 비번, 기타 등 
- 예매 
...
CREATE TABLE `movie` (
	`mo_num`	int	NOT NULL,
	`mo_titmoviemoviele`	varchar(50)	NULL,
...
CREATE TABLE `genre` (
	`ge_anme`	varchar(50)	NOT NULL
);

CREATE TABLE `movie_genre` (
	`mg_num`	int	NOT NULL,
	`mg_ge_anme`	varchar(50)	NOT NULL,
