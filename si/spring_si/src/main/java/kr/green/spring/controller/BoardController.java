...
//		System.out.println(board);
	...
	public ModelAndView boardMsodify(ModelAndView mv, Integer bd_num,HttpServletRequest request) {
		//게시글을 검색해서 화면에 전달 => 서비스에게 일을 시키는것
//		System.out.println("게시글 번호 :" +bd_num);//게시글 번호 확인
		//방법1
		//서비스에게 번호와 로그인 회원정보를 알려주면서 번호와 작성자가 일치하는 게시글을 가져오라고 시켜야됨
		//컨트롤러는 서비스가 보내준 게시글 정보를 가지고 정상접근인지 아닌지 확인
		MemberVO user = (MemberVO) request.getSession().getAttribute("user");
...
//		System.out.println(board);
	...
	public ModelAndView boardMsodifyPost(ModelAndView mv, BoardVO board) {
