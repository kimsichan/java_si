...
public class BoardController {
	...
	public ModelAndView boardList(ModelAndView mv, Criteria cri) {
		...
		cri.setPerPageNum(5); // 페이지마다 5개씩 가져온다
		//등록된 게시글 중 현재 페이지와 일치하는 게시글을 가져옴
		List<BoardVO> list = boardService.getBoardList(cri); // 게시글 목록들을 가져온다
		
		//페이지메이커를 만들어서 화면에 전달해야함
		int totalCount = boardService.getTotalCount(cri); // 총개수를 카운트한다
	...
	public ModelAndView boardRegisterPost(ModelAndView mv, BoardVO board, 
		...
		//로그인할떄 저장된 세션 user정보를 가져온다
		board.setBd_me_id(user.getMe_id());
		// 보드VO에 작성자를 로그인 user아이디로 저장한다
		...
		if( board.getBd_type().equals("공지") && authorityAdmin.indexOf(user.getMe_authority()) < 0) {
			// 보드타입이 공지이면서 로그인 user권한이 회원인경우 
			// 작성시 회원이면 작성이 안되고 공지사항 리스트로 이동한다
			mv.addObject("type","공지");
			mv.setViewName("redirect:/board/list");
		}else {
			// 게시글 작성 후 게시글 리스트로 이동
